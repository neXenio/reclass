...
def make_db_options_group(parser, defaults={}):
    ...
    ret.add_option(
        ...
        dest="ignore_class_notfound_regexp",
        default=defaults.get(
            "ignore_class_notfound_regexp", OPT_IGNORE_CLASS_NOTFOUND_REGEXP
        ),
...
def make_modes_options_group(
    ...
    ret = optparse.OptionGroup(
        parser, "Modes", "Specify one of these to determine what to do."
    )
    ret.add_option(
        inventory_shortopt,
        inventory_longopt,
        action="callback",
        callback=_mode_checker_cb,
        help=inventory_help,
    )
    ret.add_option(
        nodeinfo_shortopt,
...
def make_parser_and_checker(
    ...
    parser.usage = "%prog [options] ( {0} | {1} {2} )".format(
        inventory_longopt, nodeinfo_longopt, nodeinfo_dest.upper()
    )
    ...
    def option_checker(options, args):
        ...
        elif not hasattr(options, "mode") or options.mode not in (
            MODE_NODEINFO,
            MODE_INVENTORY,
        ):
            parser.error(
                "You need to specify exactly one mode "
                "({0} or {1})".format(inventory_longopt, nodeinfo_longopt)
            )
        elif options.mode == MODE_NODEINFO and not getattr(
            options, nodeinfo_dest, None
        ):
            parser.error(
                "Mode {0} needs {1}".format(nodeinfo_longopt, nodeinfo_dest.upper())
            )
...
def find_and_read_configfile(filename=CONFIG_FILE_NAME, dirs=CONFIG_FILE_SEARCH_PATH):
    for d in dirs:
