...
class TestEntity(unittest.TestCase):
    ...
    def test_unequal_empty_uri(self, **types):
        ...
        uri = "test://uri"
        self.assertNotEqual(
            Entity(SETTINGS, *instances, uri=uri),
    ...
    def test_unequal_types(self, **types):
        instances = self._make_instances(**types)
        self.assertNotEqual(Entity(SETTINGS, *instances, name="empty"), None)
        for i in instances:
...
class TestEntityNoMock(unittest.TestCase):
    def test_interpolate_list_types(self):
        ...
        node1_parameters = Parameters(
            {"alpha": ["${two}", "${three}"], "one": 1, "two": 2, "three": 3},
            SETTINGS,
            "first",
        )
        node1_entity = Entity(
    ...
    def test_reference_to_an_export(self):
        ...
        node3_exports = Exports({"a": "${a}", "b": "${b}"}, SETTINGS, "")
        node3_parameters = Parameters(
            {"name": "node3", "ref": "${exp}", "a": "${c}", "b": 5}, SETTINGS, ""
        )
        ...
        res_params = {
            ...
            "b": 5,
            "name": "node3",
            "exp": {"node1": 1, "node3": 3, "node2": 3},
            "ref": {"node1": 1, "node3": 3, "node2": 3},
        }
    ...
    def test_exports_multiple_nodes(self):
        ...
        node1_parameters = Parameters(
            {"name": "node1", "a": {"test": "${b}"}, "b": 1, "exp": "$[ exports:a ]"},
            SETTINGS,
            "",
        )
        node1_entity = Entity(
        ...
        node2_parameters = Parameters(
            {"name": "node2", "a": {"test": "${b}"}, "b": 2}, SETTINGS, ""
        )
        node2_entity = Entity(
            SETTINGS,
            classes=None,
    ...
    def test_exports_with_ancestor_references(self):
        inventory = {
            "node1": {"alpha": {"beta": {"a": 1, "b": 2}}},
            "node2": {"alpha": {"beta": {"a": 3, "b": 4}}},
        }
    ...
    def test_exports_failed_render(self):
        node1_exports = Exports({"a": "${a}"}, SETTINGS, "")
        node1_parameters = Parameters(
            {"name": "node1", "a": 1, "exp": "$[ exports:a ]"}, SETTINGS, ""
        )
        node1_entity = Entity(
            SETTINGS,
            classes=None,
    ...
    def test_exports_failed_render_ignore(self):
        node1_exports = Exports({"a": "${a}"}, SETTINGS, "")
        node1_parameters = Parameters(
            {"name": "node1", "a": 1, "exp": "$[ +IgnoreErrors exports:a ]"},
            SETTINGS,
            "",
        )
        node1_entity = Entity(
