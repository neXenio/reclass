...
class ExternalNodeStorage(ExternalNodeStorageBase):
    ...
    def __init__(self, nodes_uri, classes_uri, compose_node_name):
        ...
        self._nodes_storage = get_storage(
            self._nodes_uri.storage_type,
            self._nodes_uri.options,
            None,
            compose_node_name,
        )
        self._classes_default_uri = self._uri(classes_uri)
        self._classes_default_storage = get_storage(
            self._classes_default_uri.storage_type,
            None,
            ...
            for override in classes_uri["env_overrides"]:
                for (env, options) in iteritems(override):
                    ...
                    self._classes_storage[env] = get_storage(
                        uri.storage_type, None, uri.options, compose_node_name
                    )
