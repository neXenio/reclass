...
class TestCore(unittest.TestCase):
    def _core(self, dataset, opts={}, class_mappings=[]):
        ...
        storage = get_storage(
            "yaml_fs", nodes_uri, classes_uri, settings.compose_node_name
        )
    ...
    def test_ignore_class_notfound(self):
        ...
        params = {
            ...
            "_reclass_": {
                ...
                "name": {
                    "full": "class_notfound",
                    "parts": ["class_notfound"],
                    "path": "class_notfound",
                    "short": "class_notfound",
                },
            },
        }
        self.assertEqual(node["parameters"], params)
    ...
    def test_raise_class_notfound_with_regexp(self):
        reclass = self._core(
            ...
            opts={
                "ignore_class_notfound": True,
                "ignore_class_notfound_warning": False,
                "ignore_class_notfound_regexp": "notmatched.*",
            },
        )
        with self.assertRaises(ClassNotFound):
            node = reclass.nodeinfo("class_notfound")
    ...
    def test_class_mappings_match_path_false(self):
        reclass = self._core(
            "04",
            opts={"class_mappings_match_path": False},
            class_mappings=["node*    two", "alpha/node*    three"],
        )
    ...
    def test_class_mappings_match_path_true(self):
        reclass = self._core(
            "04",
            opts={"class_mappings_match_path": True},
            class_mappings=["node*    two", "alpha/node*    three"],
        )
        node = reclass.nodeinfo("node1")
        params = {
