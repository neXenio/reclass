...
class TestParameters(unittest.TestCase):
    def _construct_mocked_params(self, iterable=None, settings=SETTINGS):
        ...
        p._base.__repr__ = mock.MagicMock(
            autospec=dict.__repr__, return_value=repr(base)
        )
    ...
    def test_construct_wrong_type(self):
        ...
        self.assertIn(
            str(e.exception),
            [
                "Cannot merge <type 'str'> objects into Parameters",  # python 2
                "Cannot merge <class 'str'> objects into Parameters",
            ],
        )  # python 3
...
class TestParametersNoMock(unittest.TestCase):
    ...
    def test_interpolate_single(self):
        v = 42
        d = {"foo": "bar".join(SETTINGS.reference_sentinels), "bar": v}
    ...
    def test_interpolate_multiple(self):
        ...
        d = {
            ...
            + "meep".join(SETTINGS.reference_sentinels),
            "bar": v[0],
            "meep": v[1],
        }
        p = Parameters(d, SETTINGS, "")
    ...
    def test_interpolate_list(self):
        l = [41, 42, 43]
        d = {"foo": "bar".join(SETTINGS.reference_sentinels), "bar": l}
        p = Parameters(d, SETTINGS, "")
        p.interpolate()
    ...
    def test_complex_reference_overwriting(self):
        p1 = Parameters(
            {"one": "abc_123_${two}_${three}", "two": "XYZ", "four": 4}, SETTINGS, ""
        )
        p2 = Parameters(
            {"one": "QWERTY_${three}_${four}", "three": "999"}, SETTINGS, ""
        )
    ...
    def test_nested_reference_with_overwriting(self):
        p1 = Parameters(
            {"one": {"a": 1, "b": 2, "z": "a"}, "two": "${one:${one:z}}"}, SETTINGS, ""
        )
    ...
    def test_merge_referenced_lists(self):
        p1 = Parameters(
            {"one": [1, 2, 3], "two": [4, 5, 6], "three": "${one}"}, SETTINGS, ""
        )
    ...
    def test_interpolate_escaping(self):
        ...
        d = {
            "foo": SETTINGS.escape_character + "bar".join(SETTINGS.reference_sentinels),
            "bar": "unused",
        }
        p = Parameters(d, SETTINGS, "")
        p.initialise_interpolation()
    ...
    def test_list_merging_with_refs_for_multiple_nodes(self):
        ...
        n1 = Parameters({"name": "node1"}, SETTINGS, "")
        r1 = {
            "alpha": {"one": [1, 2], "two": [3, 4]},
            "beta": {"three": [1, 2]},
            "name": "node1",
        }
        r2 = {
            "alpha": {"one": [1, 2], "two": [3, 4]},
            "beta": {"three": [1, 2, 3, 4, 1, 2]},
            "name": "node2",
        }
        n2 = Parameters({"name": "node2"}, SETTINGS, "")
        n2.merge(p1)
    ...
    def test_nested_refs_with_multiple_nodes(self):
        ...
        r1 = {
            "alpha": {"one": 1, "two": 2},
            "beta": {"three": "one", "four": 1},
            "name": "node1",
        }
        r2 = {
            "alpha": {"one": 1, "two": 2},
            "beta": {"three": "two", "four": 2},
            "name": "node2",
        }
        n1.merge(p1)
        n1.merge(p4)
    ...
    def test_nested_refs_error_message(self):
        ...
        p1 = Parameters(
            {"alpha": {"one": 1, "two": 2}, "gamma": "${alpha:${beta}}"}, SETTINGS, ""
        )
    ...
    def test_ignore_overwriten_missing_reference_dict(self):
        ...
        p1.merge(p2)
        p1.merge(p3)
        with self.assertRaises(InterpolationError) as error, mock.patch(
            "sys.stderr", new=MockDevice()
        ) as std_err:
            p1.interpolate()
        self.assertEqual(
            error.exception.message, "-> \n   Cannot resolve ${beta}, at alpha"
    ...
    def test_complex_overwrites_1(self):
        ...
        p1 = Parameters(
            {
                "test": {"dict": {"a": "${values:one}", "b": "${values:two}"}},
                "values": {"one": 1, "two": 2, "three": {"x": "X", "y": "Y"}},
            },
            SETTINGS,
            "",
        )
        ...
        r = {
            "test": {"dict": {"a": 1, "b": {"x": "X", "y": "Y"}, "c": 2}},
            "values": {"one": 1, "three": {"x": "X", "y": "Y"}, "two": 2},
        }
        p2.merge(p3)
        p1.merge(p2)
        p1.interpolate()
    ...
    def test_escaped_string_in_ref_dict_overwrite(self):
        p1 = Parameters(
            {"a": {"one": "\${not_a_ref}"}, "b": {"two": "\${also_not_a_ref}"}},
            SETTINGS,
            "",
        )
        p2 = Parameters({"c": "${a}"}, SETTINGS, "")
        ...
        r = {
            "a": {"one": "${not_a_ref}"},
            "b": {"two": "${also_not_a_ref}"},
            "c": {"one": "${again_not_a_ref}", "two": "${also_not_a_ref}"},
        }
        p1.merge(p2)
        p1.merge(p3)
        p1.merge(p4)
