...
class Settings(object):
    ...
    known_opts = {
        ...
        "dict_key_override_prefix": defaults.PARAMETER_DICT_KEY_OVERRIDE_PREFIX,
        "dict_key_constant_prefix": defaults.PARAMETER_DICT_KEY_CONSTANT_PREFIX,
        "escape_character": defaults.ESCAPE_CHARACTER,
        "export_sentinels": defaults.EXPORT_SENTINELS,
        "inventory_ignore_failed_node": defaults.OPT_INVENTORY_IGNORE_FAILED_NODE,
        "inventory_ignore_failed_render": defaults.OPT_INVENTORY_IGNORE_FAILED_RENDER,
        "reference_sentinels": defaults.REFERENCE_SENTINELS,
        "ignore_class_notfound": defaults.OPT_IGNORE_CLASS_NOTFOUND,
        "strict_constant_parameters": defaults.OPT_STRICT_CONSTANT_PARAMETERS,
        "ignore_class_notfound_regexp": defaults.OPT_IGNORE_CLASS_NOTFOUND_REGEXP,
        "ignore_class_notfound_warning": defaults.OPT_IGNORE_CLASS_NOTFOUND_WARNING,
        "ignore_overwritten_missing_references": defaults.OPT_IGNORE_OVERWRITTEN_MISSING_REFERENCES,
    ...
    def __init__(self, options={}):
        ...
        self.dict_key_prefixes = [
            str(self.dict_key_override_prefix),
    ...
    def __eq__(self, other):
        if isinstance(other, type(self)):
            return all(
                getattr(self, opt) == getattr(other, opt) for opt in self.known_opts
            )
        return False
