...
class DuplicateUriError(ConfigError):
    ...
    def _get_message(self):
        return (
            "The inventory URIs must not be the same "
...
class UriOverlapError(ConfigError):
    ...
    def _get_message(self):
        msg = (
            "The URIs for the nodes and classes inventories must not "
...
class ClassNotFound(InterpolationError):
    ...
    def _get_error_message(self):
        msg = [
            "In {0}".format(self.uri),
            "Class {0} not found under {1}://{2}".format(
                self.name, self.storage, self.path
            ),
        ]
        return msg
...
class ClassNameResolveError(InterpolationError):
    def __init__(self, classname, nodename, uri):
        super(ClassNameResolveError, self).__init__(
            msg=None, uri=uri, nodename=nodename
        )
        self.name = classname
...
class InvQueryClassNotFound(InterpolationError):
    ...
    def _get_error_message(self):
        msg = ["Inventory Queries:", "-> {0}".format(self.classNotFoundError.nodename)]
...
class ResolveError(InterpolationError):
    ...
    def _get_error_message(self):
        msg = (
            "Cannot resolve {0}".format(self.reference.join(REFERENCE_SENTINELS))
            + self._add_context_and_uri()
        )
        return [msg]
...
class InvQueryError(InterpolationError):
    ...
    def _get_error_message(self):
        msg1 = (
            "Failed inv query {0}".format(self.query.join(EXPORT_SENTINELS))
            + self._add_context_and_uri()
        )
...
class ParseError(InterpolationError):
    ...
    def _get_error_message(self):
        msg = [
            "Parse error: {0}".format(self._line.join(EXPORT_SENTINELS))
            + self._add_context_and_uri()
        ]
...
class InfiniteRecursionError(InterpolationError):
    ...
    def _get_error_message(self):
        msg = [
            "Infinite recursion: {0}".format(self.ref.join(REFERENCE_SENTINELS))
            + self._add_context_and_uri()
        ]
        return msg
...
class BadReferencesError(InterpolationError):
    def __init__(self, refs, context, uri):
        super(BadReferencesError, self).__init__(
            msg=None, context=context, uri=uri, tbFlag=False
        )
        self.refs = [r.join(REFERENCE_SENTINELS) for r in refs]

    def _get_error_message(self):
        msg = [
            "Bad references" + self._add_context_and_uri(),
            "   " + ", ".join(self.refs),
        ]
        return msg
...
class TypeMergeError(InterpolationError):
    ...
    def _get_error_message(self):
        msg = [
            "Cannot merge {0} over {1}".format(self.type1, self.type2)
            + self._add_context_and_uri()
        ]
        return msg
...
class ExpressionError(InterpolationError):
    ...
    def _get_error_message(self):
        msg = [
            "Expression error: {0}".format(self._error_msg)
            + self._add_context_and_uri()
        ]
        return msg
...
class DuplicateNodeNameError(NameError):
    ...
    def _get_message(self):
        msg = (
            "{0}: Definition of node '{1}' in '{2}' collides with "
