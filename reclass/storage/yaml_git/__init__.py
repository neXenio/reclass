...
class GitURI(object):
    ...
    def update(self, dictionary):
        ...
        if "cache_dir" in dictionary:
            self.cache_dir = dictionary["cache_dir"]
        if "lock_dir" in dictionary:
            self.lock_dir = dictionary["lock_dir"]
        if "pubkey" in dictionary:
            self.pubkey = dictionary["pubkey"]
        if "privkey" in dictionary:
            self.privkey = dictionary["privkey"]
    ...
    def __repr__(self):
        return "<{0}: {1} {2} {3}>".format(
            self.__class__.__name__, self.repo, self.branch, self.root
        )
...
class GitRepo(object):
    def __init__(self, uri, node_name_mangler, class_name_mangler):
        ...
        if uri.cache_dir is None:
            self.cache_dir = "{0}/{1}/{2}".format(
                os.path.expanduser("~"), ".reclass/cache/git", self.name
            )
        else:
            self.cache_dir = "{0}/{1}".format(uri.cache_dir, self.name)
        if uri.lock_dir is None:
            self.lock_file = "{0}/{1}/{2}".format(
                os.path.expanduser("~"), ".reclass/cache/lock", self.name
            )
        else:
    ...
    def _fetch(self):
        ...
        for remote_branch_name in remote_branches:
            ...
            remote_branch = self.repo.lookup_branch(
                remote_branch_name, pygit2.GIT_BRANCH_REMOTE
            )
            if local_branch_name not in local_branches:
                local_branch = self.repo.create_branch(
                    local_branch_name, self.repo[remote_branch.target.hex]
                )
        ...
        for bname in self.branches:
            ...
            for file in files:
                if file.name.endswith(FILE_EXTENSION):
                    ...
                    if name in ret:
                        raise reclass.errors.DuplicateNodeNameError(
                            self.url + " - " + bname, name, ret[name], file
                        )
                    else:
                        branch[name] = file
            ret[bname] = branch
    ...
    def nodes(self, branch, subdir):
        ...
        for (name, file) in iteritems(self.files[branch]):
            if subdir is None or name.startswith(subdir):
                ...
                if node_name in ret:
                    raise reclass.errors.DuplicateNodeNameError(
                        self.url, name, ret[node_name].path, file.path
                    )
                else:
                    ret[node_name] = file
        return ret
...
class ExternalNodeStorage(ExternalNodeStorageBase):
    def __init__(self, nodes_uri, classes_uri, compose_node_name):
        ...
        if nodes_uri is not None:
            ...
            self._nodes = self._repos[self._nodes_uri.repo].nodes(
                self._nodes_uri.branch, self._nodes_uri.root
            )
    ...
    def get_node(self, name, settings):
        ...
        uri = "git_fs://{0} {1} {2}".format(
            self._nodes_uri.repo, self._nodes_uri.branch, file.path
        )
        pathname = os.path.splitext(file.path)[0]
        entity = YamlData.from_string(blob.data, uri).get_entity(
    ...
    def get_class(self, name, environment, settings):
        ...
        if uri.repo not in self._repos:
            raise reclass.errors.NotFoundError(
                "Repo " + uri.repo + " unknown or missing"
            )
        if uri.branch not in self._repos[uri.repo].files:
            raise reclass.errors.NotFoundError(
                "Branch " + uri.branch + " missing from " + uri.repo
            )
        if name not in self._repos[uri.repo].files[uri.branch]:
            raise reclass.errors.NotFoundError(
                "File " + name + " missing from " + uri.repo + " branch " + uri.branch
            )
    ...
    def _load_repo(self, uri):
        if uri.repo not in self._repos:
            self._repos[uri.repo] = GitRepo(
                uri, self.node_name_mangler, self.class_name_mangler
            )
